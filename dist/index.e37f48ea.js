...
const controlAddRecipe = async function(newRecipe) {
    try {
        ...
        setTimeout(()=>{
            if (!(0, _addRecipeViewJsDefault.default)._window.classList.contains("hidden")) (0, _addRecipeViewJsDefault.default).toggleWindow();
            // Render clean form after closing
            setTimeout(()=>{
                (0, _addRecipeViewJsDefault.default).renderForm();
            }, 300);
...
const uploadRecipe = async function(newRecipe) {
    try {
        ...
        ingredientEntries.forEach(([key, value])=>{
            ...
            if (!ingredientsByNumber[number]) ingredientsByNumber[number] = {};
...
parcelHelpers.export(exports, "MODAL_RENDER_FORM_SEC", ()=>MODAL_RENDER_FORM_SEC);
parcelHelpers.export(exports, "validationRules", ()=>validationRules);
const API_URL = "https://forkify-api.herokuapp.com/api/v2/recipes/";
const TIMEOUT_SEC = 10;
const RES_PER_PAGE = 10;
const API_KEY = "84686280-10a0-4a35-92b1-a84289f28568";
const MODAL_CLOSE_SEC = 2.5;
const MODAL_RENDER_FORM_SEC = 0.5;
const validationRules = {
    ...
    cookingTime: {
        ...
        min: 15,
        max: 1000,
        message: (value, { min, max })=>`Cooking time must be a positive number, the minimum value is ${min} and maximum ${max}`
    },
    servings: {
        required: true,
        isNumber: true,
        ...
        validateIngredient: (value, formValues)=>{
            ...
            Object.keys(ingredients).forEach((index)=>{
                ...
                if (desc || unit || quantity) {
                    if (!desc) errors[`ingredient-${index}-desc`] = "Ingredient description is required.";
                    if (quantity) {
                        if (isNaN(quantity)) errors[`ingredient-${index}-quantity`] = "Quantity must be a number.";
                        else if (Number(quantity) <= 0) errors[`ingredient-${index}-quantity`] = "Quantity must be positive.";
                        if (!unit) errors[`ingredient-${index}-unit`] = "Unit is required.";
                    }
                    if (unit) {
                        if (!isNaN(unit)) errors[`ingredient-${index}-unit`] = "Do not use numbers in unit fields.";
                    }
                }
            });
            return errors;
...
function validateField(field, value, rules) {
    ...
    if (rules.required && (!value || typeof value === "string" && !value.trim())) return `${field} is required.`;
    // Check minimum length
    if (rules.minLength && value.length < rules.minLength) return typeof rules.message === "function" ? rules.message(value) : `The minimum length for ${field} is ${rules.minLength}.`;
    // Check if the value is a valid URL
    if (rules.isUrl && !isValidURL(value)) return `${field} must be a valid URL.`;
    // Check if the value is a number and within the required range
    if (rules.isNumber) {
        if (isNaN(value)) return `${field} must be a number.`;
        if (rules.min && value < rules.min) return typeof rules.message === "function" ? rules.message(value, {
            min: rules.min,
            max: rules.max
        }) : `${field} must be at least ${rules.min}.`;
        if (rules.max && value > rules.max) return typeof rules.message === "function" ? rules.message(value, {
            min: rules.min,
            max: rules.max
...
},{"regenerator-runtime/runtime":"dXNgZ","./config.js":"k5Hzs","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"l60JC":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class RecipeView extends (0, _viewJsDefault.default) {
    ...
    _errorMessage = `Bro, we couldn't find that recipe. Please try another one`;
    _successMessage = `bro`;
...
class AddRecipeView extends (0, _viewJsDefault.default) {
    ...
    toggleWindow() {
        ...
        if (!this._window.classList.contains("hidden")) this.renderForm();
    }
    ...
    _addHandlerHideWindow() {
        this._btnClose.addEventListener("click", ()=>{
            ...
            } else if (this._parentElement.querySelector(".message")) this.toggleWindow();
            else this.toggleWindow();
        });
    ...
    _validateForm(data) {
        ...
        for (const [field, value] of Object.entries(data)){
            const rules = (0, _configJs.validationRules)[field];
            if (rules && field !== "ingredients") {
                const error = (0, _helpersJs.validateField)(field, value, rules);
                if (error) errors.push(error);
            }
        }
        // Validate ingredients separately
        const ingredientErrors = (0, _configJs.validationRules).ingredients.validateIngredient(null, data);
        for (const [field, errorMessage] of Object.entries(ingredientErrors))errors.push(`${field}: ${errorMessage}`);
        return errors;
    }
    ...
    addHandlerUpload(handler) {
        this._parentElement.addEventListener("submit", (e)=>{
            ...
            if (errors.length > 0) this.renderError(errors.join("\n"));
            else handler(data);
        });
    }
    ...
    _addHandlerAddIngredient() {
        ...
        if (addIngredientBtn) addIngredientBtn.addEventListener("click", this._handleAddIngredient.bind(this));
    }
    ...
    _getIngredientCount(data) {
        ...
        while(data[`ingredient-${count + 1}-quantity`] || data[`ingredient-${count + 1}-unit`] || data[`ingredient-${count + 1}-desc`])count++;
        return count;
    }
    _generateIngredientsMarkup(count, data) {
        let markup = "";
        for(let i = 1; i <= count; i++)markup += this._generateIngredientMarkup(i, data);
        return markup;
    }
    _captureIngredients() {
        const ingredients = [];
        ...
        ingredientFields.forEach((input, index)=>{
            ...
            if (quantity || unit || desc) ingredients.push({
                quantity,
                unit,
                desc
            });
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./View.js":"5cUXS","url:../../img/icons.svg":"loVOp","../helpers.js":"hGI1E","../config.js":"k5Hzs"}]},["hycaY","aenu9"], "aenu9", "parcelRequirecdde")
